...
export const registrarManutencao = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (itens.length > 0) {
      const { error: itensError } = await context.supabase.from("historico_manutencao_itens").insert(
        itens.map((item) => ({
          historico_id: row.id,
    ...
    console.log(`[Manutencao] registrado id=${row.id} veiculo=${cabecalho.veiculo_id} itens=${itens.length}`);
...
export const excluirManutencao = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("historico_manutencao").delete().eq("id", data.id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
