  ...
  const statsQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      if (statsError) {
        ...
        const { count: buscasCount } = await supabase
      ...
      const failedSearches = (ultimasBuscas || [])
        .filter((b: any) => {
  ...
  return (
    <>
      <PageHeader
        ...
        description="Gestão e estatísticas do catálogo técnico."
      />
      <PageBody>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <StatCard
            ...
            value={stats.totalBuscas}
            icon={Search}
            description="Histórico total"
          />
          <StatCard
            title="Respondido pelo Banco"
            value={stats.pesquisasBanco}
            icon={Clock}
            description="Acesso rápido"
          />
          <StatCard
            title="Respondido pela Internet"
            value={stats.pesquisasInternet}
            icon={TrendingUp}
            description="Via Smart Search"
          />
        </div>

        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 mt-4">
          <StatCard
            title="Economia Tavily"
            value={stats.economiaTavily.toLocaleString("pt-BR", {
              style: "currency",
              currency: "USD",
            })}
            icon={TrendingUp}
            description="Créditos poupados"
          />
          <StatCard
            title="Peças Enriquecidas"
            value={stats.pecasEnriquecidas}
            icon={Database}
            description="Salvas via IA"
          />
          <StatCard
            title="Falhas de Busca"
            value={stats.buscasSemResultado.length}
            icon={AlertTriangle}
            description="Últimas sem resultado"
          />
          <StatCard
            title="Veículos Cadastrados"
            value={stats.totalVeiculos}
            icon={Database}
            description="Identificados por chassi"
          />
          <StatCard
            title="Relacionamentos"
            value={stats.totalRelacionamentos}
            icon={Database}
            description="Veículo ↔ peça"
          />
          <StatCard
            title="Diagramas"
            value={stats.totalDiagramas}
            icon={Database}
            description="Vistas explodidas"
          />
          <StatCard
            title="Cache de Buscas"
            value={stats.totalCache}
            icon={Clock}
            description="Termos memorizados"
          />
          <StatCard
            title="Taxa de Acerto do Banco"
            value={`${stats.taxaAcertoBanco}%`}
            icon={TrendingUp}
            description="Respostas sem IA"
          />
        </div>
          ...
          <Card className="border-border/60 shadow-card">
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                {stats.buscasSemResultado.length === 0 && (
